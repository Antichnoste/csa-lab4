...
def _parse_loop(args):
    return {
        "type": "loop",
        "body": [ast_to_expr(stmt) for stmt in args]
    }
...
def _parse_call(name, args):
    return {
        "type": "call",
        "name": name,
        "args": [ast_to_expr(a) for a in args],
    }
...
def parse(tokens: list) -> list:
    ...
    # Преобразуем сырые списки в словари (удобное промежуточное представление)
...
def generate_code(ast: list, env: dict) -> tuple[list[Instruction], list[int]]:
    """Генерация инструкций по обходу AST."""
...
def compile_stmt(stmt: dict, ctx: CodegenContext) -> None:
    ...
    if stype == "return":
        ...
        if ctx.current_func is None:
            ...
            ctx.emit(Opcode.RET, AddressingMode.IMMEDIATE, 0)
        return
    if stype in ("binop", "call", "in"):
        compile_expr(stmt, ctx)
        return
    if stype in ("number", "string", "var"):
        compile_expr(stmt, ctx)
        return
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python expr_to_asm.py <input.lisp> <target.bin> [target.asm]")
        return
