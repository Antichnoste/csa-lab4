...
|:---:|:---|:---|
| **24:23** | `Latch Ctrl` | Управление защелками конвейера (Stall/Flush). |
| **22:20** | `ALU Op` | Код операции для АЛУ (ADD, SUB, MUL, DIV, etc.). |
| **19:18** | `MUX A` | Выбор адреса для памяти (0: Inst Addr, 1: SP+Offs, 2: BR). |
| **17:16** | `MUX B` | Выбор 2-го операнда АЛУ (0: Immediate, 1: DMEM, 2: IN Port). |
