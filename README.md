...
1. **IF (Instruction Fetch):** Выборка инструкции из памяти программ (`IMEM`) по адресу из регистра `PC` (счетчик инкрементируется на `+1`). Инструкция защелкивается в регистр `IF/ID`.
2. **ID (Instruction Decode & Operand Fetch):** Дешифратор разделяет 32-битное машинное слово из защелки `IF/ID` на логические шины: `Opcode`, `Mode` и `Arg` (Immediate/Addr). Одновременно с этим извлекаются текущие значения архитектурных регистров `PC`, `SP` и `ACC`. Всё это состояние процессора защелкивается в `ID/EX Latch` для автономной работы следующей стадии.
3. **EX (Execute / Write-Back):** Стадия полностью управляется CU. `Opcode` и `Mode` из защелки `ID/EX` транслируются в стартовый адрес ПЗУ. Далее микрокод потактово управляет мультиплексорами, выполнением операций в `ALU`, доступом к памяти данных (`DMEM`) и регистрам (`ACC`, `SP`, `PC`). Флаги `Z` и `N` от АЛУ отправляются обратно в CU для вычисления условий ветвления.
...
- **Стадии конвейера и Защелки (Latches):**
  ...
  - **ID / EX Latch:** Сохраняет состояние процессора для автономной работы стадии EX. Фиксирует операнды, а также пробрасывает транзитом значения архитектурных регистров `PC`, `SP` и `ACC`.
- **Регистры:**
...
**Принцип работы (потактовый автомат Мура):**
1. 10 бит (`Opcode` и `Mode`) из защелки `ID/EX` поступают в комбинационный **Address Mapper**, который мгновенно транслирует их в стартовый 6-битный адрес микропрограммы.
...
4. Микрослово попадает в **Control Signal Generator**, где расплетается на физические управляющие провода (управление защелками, мультиплексорами, АЛУ и памятью), которые мгновенно применяются к DataPath.
