...
*   **Заголовок:** (Опционально) содержит информацию о размере секции кода.
...
*ПРИМЕР ВЫПОЛНЕНИЯ МИКРОКОДА ПОТАКТОВО:*
...
1. `cat.lisp` — читает поток `stream` через порты ввода-вывода и выводит его.
2. `hello.lisp` — выводит статическую `cstr` строку "Hello, World!".
3. `hello_user_name.lisp` — запрашивает имя, конкатенирует (или выводит подряд) со строкой приветствия.
4. `prob1.lisp` — Project Euler 1, алгоритм согласно варианту. Демонстрирует математику, работу с функциями, локальными переменными и стеком.

Запуск тестов: `pytest tests/`

Пример вывода симулятора (`cat.lisp`):
```text
TICK:  10 | IF: LD #0   | ID: IN 0    | EX: JMP L1   | ACC: 0  | STALL: 0
TICK:  11 | IF: CMP #0  | ID: LD #0   | EX: IN 0     | ACC: 72 | STALL: 0   <-- Чтение 'H'
TICK:  12 | IF: JZ end  | ID: CMP #0  | EX: LD #0    | ACC: 72 | STALL: 0
```
В журнале работы четко отслеживается движение инструкций по стадиям конвейера и состояние процессора на каждом такте.